...
def parse_option():
    ...
    # specify folder
    parser.add_argument("--model-path", type=str, default="./", help="path to save model")
...
def option_update(opt):
    ...
    opt.tb_folder = os.path.join('./', opt.model_name)
...
def train_finetune(
    ...
    for idx, batch in enumerate(train_loader):
        ...
        graph_q.to(torch.device(opt.gpu if torch.cuda.is_available() else "cpu"))
        y = y.to(torch.device(opt.gpu if torch.cuda.is_available() else "cpu"))
...
def train_moco(
    ...
    for idx, batch in enumerate(train_loader):
        ...
        # print(f'【train】---------graph_q:{graph_q}')
        # print(f'【train】---------graph_k:{graph_k}')
        graph_q.to(torch.device(opt.gpu if torch.cuda.is_available() else "cpu"))
        graph_k.to(torch.device(opt.gpu if torch.cuda.is_available() else "cpu"))

        # bsz = graph_q.batch_size   # 'BatchedDGLHeteroGraph' object has no attribute '_batch_size'
        bsz = opt.batch_size
...
# def main(args, trial):
def main(args):
    dgl.random.seed(args.seed) # default = 0
    ...
    if args.finetune:
    ...
    elif args.dataset == "dgl": # embedding作用的dataset
        train_dataset = LoadBalanceGraphDataset( # return 2 graphs
            rw_hops=args.rw_hops, # 256
            restart_prob=args.restart_prob, # 0.8
            positional_embedding_size=args.positional_embedding_size, # 32
            num_workers=args.num_workers, # 2
            num_samples=args.num_samples, # 2000个子图
            dgl_graphs_file="./data/small.bin",
            num_copies=args.num_copies, # 2
        )
    else: 
        if args.dataset in GRAPH_CLASSIFICATION_DSETS or args.dataset=="mydataset":
            ...
            print("======================train_dataset finished===================")
            print(f"=====rw_hops: {args.rw_hops}, subgraph_size:{args.subgraph_size}, restart_prob:{args.restart_prob}, positional_embedding_size:{args.positional_embedding_size}=====")
        else:
    ...
    print(f'batch_size = {args.batch_size}, args.num_workers={args.num_workers}')
    train_loader = torch.utils.data.DataLoader(
        dataset=train_dataset,
        batch_size=args.batch_size,  # 32 how many samples per batch to load。在自己的代码中应该改成len(k0,p, k1-k)个
        collate_fn=labeled_batcher() if args.finetune else batcher(), # if pretraining: 合并两个graph。否则就是一个graph一个label. merges a list of samples to form a mini-batch of Tensor(s)
        shuffle=False,
    ...
    # copy weights from `model' to `model_ema'. ema: 指数平均移动
    if args.moco:
